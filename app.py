...
# sort - a check to determine if the recipes returned in the search, delete or
# edit requests should be sorted into order
# filters - a list of the filters currently applied to the recipe results
# returned. Needed to ensure the filter is applied after leaving the
# search page.
...
def add_recipe():
    ...
    # return all the results from the ingredients collection to create an array
    # of ingredients names.
    ...
    # return all the results from the countries collection to create an array
    # of ingredients names.
    # This array is passed to the autocomplete function on the page
...
def edit_recipe(recipe_id):
    ...
    # get a list of ingredients from ingredients collection to display in the
    ...
    # get a list of countries from countries collection to display in the auto
    ...
    return render_template('edit_recipe.html', title_text="Edit your recipes",
                           recipe=_recipe, ingredients_list=_ingredients,
                           country_list=_countries)


@app.route("/insertrecipe/", methods=['POST'])
def insertrecipe():
    # called on editting an existing recipe or creating a new one in order to
    # update or add to the recipe collection.
    ...
    if edit == 'True':
        ...
        # if the user is not editting an existing recipe create a new entry in
        # the recipe collection
        recipe_id = recipes.insert({'name': 'new', 'favourite': []})

    # request form data is not in flat format - arrays are created in the form
    # for ingredients and allergen information
    new_recipe = request.form.to_dict(flat=False)
    # set up a list of ingredients that will be added to the recipe in the
    # collection
    ingredients = []
    # set up a dictionary for each individual ingredient which will consist of
    # the name, quantity and unit for each ingredient.
    ingredient = {}
    # initialise cook_time variable to convert hours and minutes to minutes
    # total to enable easy sorting by cook time against the recipes returned
    # from a search
    cook_time = 0

    # iterate through the form dictionary to correctly format the user input to
    # add to the new recipe document
    # each recipe ID is stored against the relevant document in each category
    # collection:
    # allergens, authors, countries, difficulty, ingredients, meals and users
    # (for favourites)

    for key, key_name in new_recipe.items():
        # flatten dict for non-list items which have become lists through
        # request object
        if key != 'type' and key != 'amount' and key != 'unit' and\
            ...
            if key == 'favourite':
                ...
                # get the list of users for whom this is a favourite recipe
                # from the recipe document 'favourite' list element
                favourites = recipe_doc["favourite"]
                # check whether a user is logged in and if this user has marked
                # this recipe as a favourite

                if "user" in session:
                    user_doc = usersdb.find_one({"name": session["user"]})
                    if new_recipe[key].lower() == 'true':
                        # if the user has  logged in and their ID is not stored
                        # in the favourites list for this recipe document then
                        # add the user ID to the favourites list
                        # also add the recipe id to the favourites list element
                        # in the user document.
                        if recipe_id not in user_doc["favourites"]:
                            usersdb.update({"name": session["user"]},
                                           {"$push":
                                           {"favourites": recipe_id}})
                            favourites.append(str(user_doc["_id"]))
                    else:
                        # if the user has not marked this recipe as a favourite
                        # and the user ID is in the favourites list for the
                        # recipe document
                        # then remove the user id  from the favourites list and
                        # remove the recipe ID from the user document
                        # favourites list
        ...
        # iterate through the ingredients submitted
            ...
            # create dictionaries for each ingredient with key values of
            # ingredient type, amount and units
            # update the list of ingredients with each ingredient dictionary
            i = 0
            for ingredient_type in key_name:
                ingredient = {'type': ingredient_type.lower(),
                              'amount': new_recipe['amount'][i],
                              'unit': new_recipe['unit'][i]}
    ...
    # the recipe dictionary is now ready to add/update the document in the
    # recipes collection

    # The recipe ID, however needs to be added to/removed from the recipe lists
    # in other collections.

    # check if the ingredients submitted are in the ingredients collection and
    # add/remove ingredients from the collection
    ...
    for ingredient in new_ingredients:
        ...
        if edit == "True":
            # if so get a list of the ingredients that are currently in the
            # recipe document (before it was editted)
            # this is to check whether the ingredients have been changed. If so
            # the ingredients may need to be added or removed from the
            # igredients collection
            current_ingredients = current_recipe["ingredients"]
            # check if each new ingredient is already in the list of
            # ingredients for this recipe.
            # if it is there is no need to take further action (it does not
            # need to be added to the ingredients collection)
        ...
        new_ingredient_doc = ingredientsdb.find_one(
            {"name": ingredient["type"].lower()})

        # for each ingredient, check whether it exists in the ingredient
        # collection.
        if new_ingredient_doc is not None:
            ...
            new_ingredient_id = ingredientsdb.insert(
                {"name": ingredient["type"].lower(), "recipe": []})
            new_ingredient_doc = ingredientsdb.find_one(
                {"_id": new_ingredient_id})
        # check if the id of the recipe is stored in the ingredient doc in
        ...
        if recipe_id not in new_ingredient_doc["recipe"]:
            ingredientsdb.update({"_id": new_ingredient_id},
                                 {"$push": {"recipe": recipe_id}})

    # the current_ingredients list now only contains those ingredients which
    # have been removed from the recipe.
    # the recipe ID needs to be removed from each recipe list in each
    # ingredient doc.
    if edit == "True":
        for ingredient in current_ingredients:
            if ingredient not in new_ingredients:
                current_ingredient_doc = ingredientsdb.find_one(
                    {"name": ingredient["type"].lower()})
                current_ingredient_id = current_ingredient_doc["_id"]
                ingredientsdb.update({"_id": current_ingredient_id},
                                     {"$pull": {"recipe": recipe_id}})
                current_ingredient_doc = ingredientsdb.find_one(
                    {"_id": current_ingredient_id})
                # if the list of recipes for this ingredient is empty then
                # this ingredient is not used in any recipes and can be removed
                # from the ingredients collection.
                if len(current_ingredient_doc["recipe"]) == 0:
                    ingredientsdb.remove({"_id": current_ingredient_id})

    # check each of the input fields - allergens, meal, country, author
    # and difficulty
    # to see if values have been changed
    categories = ["allergens", "meal", "country", "author", "difficulty"]
    for category in categories:
        # check if the category is in the recipe dict. if it is then format
        # correctly if the category itself contains a list of elements (e.g.
        # allergens)
        ...
        for value in new_category:
            # check if we are editting an existing recipe, if so check whether
            # any values exist in both the updated and current recipes
            # if so remove them from the list to later delete the recipe ID
            # from the values that remain in the list (like ingredients
            # iteration, above)
            if edit == "True":
            ...
            if value:
                if new_category_doc is not None:
                    # if the particular element exists then get the associated
                    # doc from the relevant category collection
                    new_category_id = new_category_doc["_id"]
                else:
                    # if it doesn't exist and is not blank create a new doc
                    # with the name of the element and create a recipe list
                    # in the document containing the ID of this recipe
                    new_category_id = categorydb.insert(
                        {"name": value.lower(), "recipe": []})
                    new_category_doc = categorydb.find_one(
                        {"_id": new_category_id})
                if recipe_id not in new_category_doc["recipe"]:
                    # check if the recipe is already in the new doc, if it
                    # isn't add the recipe ID to this particular category
                    # in the DB
                    categorydb.update({"_id": new_category_id},
                                      {"$push": {"recipe": recipe_id}})

        if edit == "True":
            # if we are editing a recipe check whether there are any remaining
            # values in the category list
            # if so this means that the recipe has been removed from the
            # relevant category
            if category in current_recipe:
                # for all the values remove the recipe ID from the particular
                # category in the relevant collection/
                for value in current_category:
                    if value not in new_category:
                        current_category_doc = categorydb.find_one(
                            {"name": value})
                        current_category_id = current_category_doc["_id"]
                        categorydb.update({"_id": current_category_id},
                                          {"$pull": {"recipe": recipe_id}})
                        current_category_doc = categorydb.find_one(
                            {"_id": current_category_id})
                        # if this particular category is empty, delete it
                        # from the DB
                        if len(current_category_doc["recipe"]) == 0:
                            categorydb.remove({"_id": current_category_id})

    # update the recipe document in the recipe collection with the new recipe
    # dictionary
...
def search():
    ...
    # check the category_form session variable - these are the categories
    # selected by the user to search against.
    ...
    try:
        ...
        pass
    categories = ["ingredients", "allergens", "difficulty", "meal", "country",
                  "author"]
    _category_lists = {}
    ...
    # display the search page and pass the list of items against which the user
    # can filter their results.
    return render_template("search.html", title_text="Find your favourites",
                           category_list=_category_lists)


@app.route("/recipe_list", methods=['POST', 'GET'])
def recipe_list():
    # called on selecting the search recipes option from the menu or
    # 'apply filters' button

    # need to check if the category_form session variable exists.
    # if it does it needs to be deleted so that, after displaying a recipe
    # selecting 'back' will return to recipe_list not category_search
    try:
        session.pop("category_form")
    except KeyError:
        pass

    # check if the session variable sort exists. If it does not then get the
    # value from the form and set it to the session variable.
    # the sort variable is used to sort the results list into order, depending
    # on the criteria chosen by the user on the recipe_list page
    if session.get('sort'):
        _sort = session["sort"]
    else:
    ...
    # initialise user_id variable to pass to front end to check, when
    # displaying recipe list, if a recipe is a favourite of the user's
    _user_id = ""
    if "user" in session:
    ...
    # if returning to the recipe list page from the display recipe page the
    # results will need to be pre-filtered, based on the filter criteria the
    # user
    # chose previously. This is shown by the filter variable set as 'existing'.
    # If there is a session variable for filters the filter dictionary is taken
    # from this.
    # if new filter criteria are being used (because the apply filters button
    # has been clicked) then the filter dictionary is pulled from the form
    # if neither existing or new filter criteria are being used (because the
    # page has been accessed for the first time, or the filter criteria have
    # been cleared) initialise the filter dictionary
    ...
    # get the action request from the page to determine what sort of list is to
    # be displayed and display appropriate title text
    ...
    # if the user is editting or deleting recipes then they can only edit or
    ...
    # if the user has selected filter values and/or if the user is
    # editing/deleting recipes) then create a dictionary to pass to mongo
    ...
    if filter == 'new':
        ...
        # check if each filter has input provided from the form, if not, remove
        # the filter from the dictionary
        # favourites are filtered by the recipe ID held in the user doc
        # favourites element
        ...
        # the cook time filter needs further definition. A simply int is passed
        # from the form. For filtering this needs to be
        ...
        # session variable is required for moving between pages so that, when
        # returning to the search page the filter criteria are retained.
    ...
    # create lists for dropdown filter menus for authors and countries on the
    # recipe_list page
    ...
    return render_template("recipe_list.html", title_text=title_text_value,
                           recipes=_recipe_list, countries=_country_list,
                           authors=_author_list, action=_todo,
                           filters=_filter_dict, sort=_sort,
                           total_results=_total_results, userid=_user_id)


@app.route('/display_recipe/<recipe_id>', methods=['POST', 'GET'])
def display_recipe(recipe_id):
    # called from selecting a recipe in the recipe list, display category or
    # after submitting add/edit input forms
    ...
    # check request came from add or edit recipe to ensure clicking back will
    # render the edit recipe page correctly
    _added = request.args.get('added_recipe', None)

    # initialise user_id variable to pass to front end to check if the
    # displayed recipe is a favourite of the user
    _user_id = ""

    # check if the user is logged on, if not pass false value for favourite so
    # no star is shown on display_recipe.html
    if "user" in session:
        usersdb = mongo.db.usersDB
        user_doc = mongo.db.usersDB.find_one({"name": session["user"]})
        _user_id = str(user_doc["_id"])
        ...
        if mark_favourite == 'true':
            if ObjectId(recipe_id) not in user_doc["favourites"]:
                ...
                recipedb.update({"_id": ObjectId(recipe_id)},
                                {"$push": {"favourite": _user_id}})
                _recipe = recipedb.find_one({"_id": ObjectId(recipe_id)})
        elif mark_favourite == 'false':
            # if the user has not marked this recipe as a favourite and the
            # user ID is in the favourites list for the recipe document
            # then remove the user id  from the favourites list and remove the
            # recipe ID from the user document favourites list
            if ObjectId(recipe_id) in user_doc["favourites"]:
                usersdb.update({"name": session["user"]},
                               {"$pull": {"favourites": ObjectId(recipe_id)}})
                recipedb.update({"_id": ObjectId(recipe_id)},
                                {"$pull": {"favourite": _user_id}})
                _recipe = recipedb.find_one({"_id": ObjectId(recipe_id)})
    title_text = 'Your recipe'
    return render_template('display_recipe.html', title_text='Your recipe',
                           recipe=_recipe, added=_added, userid=_user_id)
...
def delete_recipe():
    ...
    # iterate through the elements in the document and remove the recipe ID
    # from each element in each category for which the recipe has an entry
    for key in recipe:
        if key == 'favourite':
            # need to iterate through the complete list of users who have
            # marked this recipe as favourite and remove it from their
            # favourite recipe list elements in the relevant user doc
            for value in recipe[key]:
                mongo.db.usersDB.update({"_id": ObjectId(value)},
                                        {"$pull":
                                        {"favourites": ObjectId(recipe_id)}})
        if key == "meal":
            mongo.db.mealDB.update({"name": recipe[key].lower()},
                                   {"$pull": {"recipe": ObjectId(recipe_id)}})
        if key == "difficulty":
            mongo.db.difficultyDB.update({"name": recipe[key].lower()},
                                         {"$pull":
                                         {"recipe": ObjectId(recipe_id)}})
        if key == "allergens":
            # need to iterate through all allergens and remove the recipe ID
            # from each allergen document recipe list
            for value in recipe[key]:
                mongo.db.allergensDB.update({"name": value},
                                            {"$pull":
                                            {"recipe": ObjectId(recipe_id)}})
        if key == "country":
            mongo.db.countriesDB.update({"name": recipe[key].lower()},
                                        {"$pull":
                                        {"recipe": ObjectId(recipe_id)}})
            # check if there are any recipes stored against this country.
            # If not delete the country doc from the country collection
            check_and_delete_empty_doc(recipe[key].lower(),
                                       mongo.db.countriesDB)
        if key == "author":
            mongo.db.authorDB.update({"name": recipe[key].lower()}, {"$pull":
                                     {"recipe": ObjectId(recipe_id)}})
            # check if there are any recipes stored against this author. If not
            # delete the author doc from the author collection
            check_and_delete_empty_doc(recipe[key].lower(), mongo.db.authorDB)
        if key == "ingredients":
            for value in recipe[key]:
                mongo.db.ingredientsDB.update({"name": value["type"]},
                                              {"$pull":
                                              {"recipe": ObjectId(recipe_id)}})
                # check if there are any recipes stored against this
                # ingredient. If not delete the ingredient doc from the
                # ingredient collection
                check_and_delete_empty_doc(value["type"],
                                           mongo.db.ingredientsDB)
...
def check_and_delete_empty_doc(item, coll):
    ...
    # check if the document exists, if so and there are no recipes registered
    # against the document delete it from the collection
...
def display_categories():
    # called from the search page after user selects filters to determine which
    # categories are to be displayed
    # check if the user has already submitted the form listing the categories
    # to be displayed.
    # This session variable ensures, after displaying a recipe, the selected
    # categories are correctly displayed again
    try:
        selected_categories = session["category_form"]
    except KeyError:
        # if no session variable exists get the categories from the user
        # submitted form.
    ...
    # iterate through the categories selected for filtering and create lists of
    # the items in each category that have been selected.
    ...
    recipes_in_category = {}
    # set a search value, for all categories except allergens the recipes
    # returned need to contain the element searched against
    # in the case of allergens the recipes returned must not contain the
    # specific element.
    search_value = "$in"
    if category == "allergens":
    ...
    # perform mongo search against each category item and crate list of recipes
    # to display for each category.
    ...
    return render_template('display_category.html', category=category,
                           items=_item_list, recipes=recipes_in_category,
